...
- x:
  ...
  (character) For `scroll`, a single scroll id; for `scroll_clear`, one
  or more scroll id's
...
- stream_opts:
  ...
  [`jsonlite::stream_out()`](https://jeroen.r-universe.dev/jsonlite/reference/stream_in.html) -
  Except that you can't pass `x` as that's the data that's streamed out,
  and pass a file path sinstead of a connection to `con`. `pagesize`
...
- all:
  ...
  (logical) If `TRUE` (default) then all search contexts cleared. If
  `FALSE`, scroll id's must be passed to `x`
...
Lists passed to `scroll()` without a `_scroll_id` element will trigger
an error.
...
explicitly cleared as soon as the scroll is not being used anymore using
`scroll_clear`
