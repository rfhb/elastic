...
`elastic` is an R client for
[Elasticsearch](https://www.elastic.co/elasticsearch). This vignette is
an introduction to the package, while other vignettes dive into the
...
- Add shortcut: `sudo ln -s elasticsearch-6.5.3 elasticsearch` (replace
...
Windows users can follow the above, but unzip the zip file instead of
...
On package load, your base url and port are set to `http://127.0.0.1`
and `9200`, respectively. You can of course override these settings per
...
I have prepared a non-exported function useful for preparing the weird
format that Elasticsearch wants for bulk data loads (see below). See
`elastic:::make_bulk_plos` and `elastic:::make_bulk_gbif`.
...
Search the `plos` index, and the `article` document type, and query for
*antibody*, limit to 1 result
