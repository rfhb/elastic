...
test_that("docs_mget works", {
  ...
  invisible(docs_create(x, index = ind3, id = 1, body = '{"hello": "world"}'))
  invisible(docs_create(x, index = ind3, id = 2, body = '{"foo": "bar"}'))
  invisible(docs_create(x, index = ind3, id = 3, body = '{"tables": "chairs"}'))
  e <- docs_mget(x, index = ind3, ids = 1:3, verbose = FALSE)
  expect_is(e, "list")
  expect_named(e, "docs")
...
test_that("docs_delete works", {
  ...
  f <- docs_delete(x, index = ind3, id = 3)
  expect_is(f, "list")
  if (x$es_ver() < 600) {
    expect_true(f$found)
  } else {
  ...
  expect_error(docs_delete(x, index = ind3, id = 3), "Not Found")
})
...
test_that("document ids with spaces work", {
  ...
  # create
  f <- docs_create(x, index = ind4, id = "hello world", body = list(a = "asdfasdf"))
  invisible(docs_create(x, index = ind4, id = "foo bar", body = list(a = "asdfadfadfasdfasdfsdf")))
  if (x$es_ver() < 600) {
  ...
  # get
  g <- docs_get(x, index = ind4, id = "hello world", verbose = FALSE)
  expect_is(g, "list")
  expect_true(g$found)

  # mget - ids param
  h <- docs_mget(x, index = ind4, ids = c("hello world", "foo bar"), verbose = FALSE)
  expect_is(h, "list")
  ...
  invisible(docs_create(x, index = ind5, id = "hello mars", body = list(radius = 1000000L)))
  ...
  if (x$es_ver() < 600) {
    ...
    # update
    i <- docs_update(x, index = ind4, id = "hello world", body = list(doc = list(a = "an update")))
    expect_is(i, "list")

    # delete
    j <- docs_delete(x, index = ind4, id = "hello world")
    expect_is(j, "list")
    expect_true(j$found)
  }
})
